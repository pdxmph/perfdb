...
base_dir = File.dirname(__FILE__) + "/../import_files/spreadsheets/recon/"
...
spreadsheets.each do |s|
  ...
  sheet.each 1 do |row|
    ...
    begin
      art_domain = URI.parse(art_url).host
    ...
    begin
      ...
      article = Article.find_by_title(row[1])
      article.author = Author.find_or_create_by_name(row[3])
      article.url = row[8]
      article.pub_date =  row[2]
      article.content_type = ContentType.find_by_code(row[4][0,1].upcase)
      article.content_source = ContentSource.find_by_code(row[5][0,1].upcase)
